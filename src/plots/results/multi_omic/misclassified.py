...
def process_predictions(base_dir, tool, fusion, validation_mode, model):
    target_folder = None
...
def build_matrix_for_mode(base_dir, validation_mode, model):
    tools = ['geN', 'VIB', 'VS2']
    fusions = ['early', 'late']
    fusions_map = {'early': 'Early Fusion', 'late': 'Late Fusion'}
    matrices_by_tool = {}
    for tool in tools:
        all_samples = set()
        fusion_data_dict = {}
        for fusion in fusions:
            res = process_predictions(base_dir, tool, fusion, validation_mode, model=model)
            if res is not None:
                fusion_data_dict[fusion] = res
                all_samples.update(res['clean_sample_id'].tolist())
        if not fusion_data_dict:
            continue
        sorted_samples = sorted(list(all_samples), key=lambda x: extract_sample_number(x))
        matrix_data = []
        row_labels = []
        for fusion in fusions:
            if fusion in fusion_data_dict:
                res = fusion_data_dict[fusion]
                sample_to_val = dict(zip(res['clean_sample_id'], res['is_misclassified']))
                row = [sample_to_val.get(s, 0.0) for s in sorted_samples]
                matrix_data.append(row)
                row_labels.append(fusions_map[fusion])
            else:
                matrix_data.append([0.0] * len(sorted_samples))
                row_labels.append(f"{fusions_map[fusion]} (No Data)")
    ...
    for mode in modes:
        ...
        for t_idx, tool in enumerate(tools):
            ...
            if tool in matrices:
                ...
                ax.set_ylabel(tools_map[tool], fontsize=14, fontweight='bold', labelpad=18)
            else:
        ...
        output_filename = f"plots/results/multi_omic/{model}/misclassified_{mode}.pdf"
