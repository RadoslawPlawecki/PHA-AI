...
def generate_confusion_matrix_grid(base_dir, validation_mode, model):
    use_latex()
    fig, axes = plt.subplots(2, 3, figsize=(10.5, 7.5), layout="constrained")
    rows = ['early', 'late']
    cols = ['geN', 'VIB', 'VS2']
    cols_map = {'geN': 'geNomad', 'VIB': 'VIBRANT', 'VS2': 'VirSorter2'}
    rows_map = {'early': 'Early Fusion', 'late': 'Late Fusion'}
    ...
    for r_idx, r in enumerate(rows):
        for c_idx, c in enumerate(cols):
            ...
            for folder in all_folders:
                if folder.startswith(f"run_{r}_fusion_{c}_"):
            ...
            if target_folder:
                ...
                if os.path.exists(cm_path):
                    ...
                    if c_idx == 2:
                        ax.set_ylabel("True Class", fontsize=17, fontweight='bold', rotation=270, labelpad=-185)
                    else:
                        ax.set_xlabel("")
                    if r_idx == 0:
    ...
    plt.tight_layout()
    fig.subplots_adjust(top=0.88, hspace=0.15, wspace=0.55)
    output_filename = f"plots/results/multi_omic/{model}/confusion_matrix_grid_{validation_mode}.pdf"
