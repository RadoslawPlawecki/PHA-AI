...
def find_feature_importance_file(base_dir, tool, mode, model):
    target_folder = None
    if os.path.exists(base_dir):
        for folder in os.listdir(base_dir):
            if folder.startswith(f"run_early_fusion_{tool}_"):
...
def generate_importance_plots(base_dir, model):
    ...
    for row_idx, tool in enumerate(tools):
        for col_idx, mode in enumerate(modes):
            ...
            file_path = find_feature_importance_file(base_dir, tool, mode, model)
            if file_path:
                has_any_data = True
