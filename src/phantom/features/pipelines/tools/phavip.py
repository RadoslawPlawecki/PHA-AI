...
class PhavipFeaturePipeline:
    ...
    CATEGORY_PATTERNS: ClassVar[dict] = {
        "structural": r"capsid|head|tail|portal|fiber|baseplate|"
        r"sheath|spike|scaffolding|neck|virion|coat|"
        r"tape measure|connector|pilot",
        "packaging": r"terminase|packaging",
        "lysis": r"holin|endolysin|amidase|lysin|spanin|"
        r"peptidoglycan|murein",
        "lysogeny": r"integrase|repressor|anti-repressor|cro|"
        r"cII|rdf|directionality factor|excisionase|invertase",
        "replication_recombination": r"polymerase|primase|helicase|ligase|"
        ...
        "transcription_regulation": r"transcription|rna polymerase|sigma|"
        r"transcriptional activator|transcriptional regulator|"
        r"helix-turn-helix|hth|gemA",
        "host_takeover": r"histone-like|hu/ihf|rnapi associated|"
        r"nuclear pore|chromatin|host takeover",
        "nucleotide_metabolism": r"dutp|diphosphatase|ribonucleotide reductase|"
        r"kinase|phosphatase|queuosine|sam|"
        r"cyclohydrolase",
        "defense_accessory": r"methyltransferase|restriction|toxin|antitoxin|"
        r"reverse transcriptase|parA|parB|transposase",
        "maturation_accessory": r"protease|peptidase",
        "metabolism_accessory": r"isomerase|hydrolase|oxidoreductase|reductase",
        "unknown": r"duf|hypothetical|orf|gp\d+|vog",
    }
    ...
    ALL_CATEGORIES: ClassVar[list] = [*CATEGORY_PATTERNS, "other_function"]

    def __init__(self, min_coverage: float = 0.7, min_pident: float = 0.35):
        self.min_coverage = min_coverage
        self.min_pident = min_pident
        self.compiled_patterns = {
            k: re.compile(v, re.IGNORECASE) for k, v in self.CATEGORY_PATTERNS.items()
        }
    ...
    def preprocess(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df = df[(df["coverage"] >= self.min_coverage) & (df["pident"] / 100 >= self.min_pident)]
