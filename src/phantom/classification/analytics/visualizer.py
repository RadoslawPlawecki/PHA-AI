...
class Visualizer:
    ...
    @staticmethod
    def plot_unsupervised_grid(
        coords_dict, y_aligned, sample_ids, title_main="Multi-Dimensional Scaling", save_path=None
    ):
        use_latex()
        _fig, axs = plt.subplots(2, 2, figsize=(14, 8))
        ...
        for (row, col), (key, title) in config.items():
            ...
            for i, (x, y) in enumerate(coords):
                text_to_display = (
                    str(sample_ids[i]).split("|S")[1].strip()
            ...
            for index, name in enumerate(class_labels):
                ax.scatter(
                    [], [], c=["#1f77b4", "#d62728"][index], label=name, edgecolors="k", s=75
                )
            ax.legend(loc="best")
